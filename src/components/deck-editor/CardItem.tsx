...
import type { ScryfallCardResponse } from "../../types";
...
export const CardItem: React.FC<CardItemProps> = ({ card, isInDeck, currentQuantity, canAddMore, onAdd }) => {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h4 className="text-sm font-medium text-gray-900 truncate">{card.name}</h4>
              <p className="text-xs text-gray-600 mt-1">{formatCardType(card.type)}</p>
              {card.mana_cost && <p className="text-xs text-gray-500 mt-1">Koszt: {formatManaCost(card.mana_cost)}</p>}
              {card.rarity && <p className="text-xs text-gray-500">Rzadkość: {card.rarity}</p>}
            </div>
            ...
            <div className="flex-shrink-0 ml-4">
              {isInDeck ? (
                <div className="text-center">
                  <div className="text-sm text-gray-600 mb-1">W decku: {currentQuantity}</div>
                  {canAddMore ? (
                    <Button onClick={onAdd} size="sm" className="bg-green-600 hover:bg-green-700">
                      + Dodaj
                    </Button>
                  ) : (
                    <div className="text-xs text-gray-500">Limit osiągnięty</div>
                  )}
                </div>
              ) : (
