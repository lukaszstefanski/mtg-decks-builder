...
import type { DeckState, CardResponse, AddCardToDeckRequest, UpdateDeckCardRequest, DeckCardResponse } from '../types';
...
export const useDeckCards = (deckId: string) => {
   ...
   */
  const addCard = useCallback(async (card: CardResponse, quantity: number = 1, notes?: string) => {
    try {
      setState(prev => ({ ...prev, loading: true, error: null }));
   ...
   */
  const updateCard = useCallback(async (cardId: string, data: UpdateDeckCardRequest) => {
    try {
      setState(prev => ({ ...prev, loading: true, error: null }));
      ...
      setState(prev => ({
        ...
        totalCount: prev.totalCount + (data.quantity || 0) - 
          (prev.cards.find(c => c.id === cardId)?.quantity || 0),
        loading: false,
      }));
   ...
   */
  const canAddMoreCards = useCallback((cardId: string, cardType: string) => {
  ...
  const getDeckStats = useCallback(() => {
    ...
    const lands = state.cards.filter(card => 
